    ...
    db: function(name, db_opts) {
      ...
      return /** @lends $.couch.db */{
        ...
        openDoc: function(docId, options, ajaxOptions) {
          ...
          if (options.show) {
            show_parts = options.show.split('/');
            url = this.uri + '_design/' + show_parts[0] + '/_show/' + show_parts[1] + '/';
            delete options[show]
          } else {
            url = this.uri
          }
          ...
          ajax({url: url + encodeDocId(docId) + encodeOptions(options)},
            options,
        ...
        saveDoc: function(doc, options) {
          ...
          if (options.update) {
            update_parts = options.update.split('/');
            url = this.uri + '_design/' + update_parts[0] + '/_update/' + update_parts[1] + '/';
          ...
          if (doc._id === undefined) {
            ...
            var method = "PUT";
            url += encodeDocId(doc._id);
          }
          var versioned = maybeApplyVersion(doc);
          $.ajax({
            type: method, url: url + encodeOptions(options),
